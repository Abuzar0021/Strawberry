...
/**
 * Prepares a plate's moving version.
 *
...
export function loadMotion(plate: Plate): Promise<HTMLVideoElement> {
  return new Promise((resolve, reject) => {
    if (!plate.motion) return reject(new Error("no motion source"));
    const v = document.createElement("video");
    v.src = plate.motion;
    v.muted = true;
    // a scrubbed clip is positioned by hand; looping would fight the playhead
    v.loop = !plate.scrub;
    v.playsInline = true;
    v.preload = "auto";
